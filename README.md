...
### JWT Token Update (December 2025)

**Important**: Recent changes improve JWT token security and reliability by including user identifiers in token payloads.

#### What Changed
- JWT tokens now include `id`, `user_id`, and `email` fields in the payload
- Authentication middleware now supports multiple token formats:
  - Standard fields: `id`, `user_id`, `sub` (JWT standard)
  - Nested format: `user.id`, `user.email`
- Token expiration now uses `JWT_EXPIRES_IN` from environment config (default: 7 days)
- Enhanced logging for token verification debugging

#### Required Actions After Deployment
1. **Users must re-login**: Existing tokens without user ID will be rejected
2. **Update client applications**: Ensure clients handle 401 responses gracefully
3. **Verify environment variables**: Check that `JWT_SECRET` and `JWT_EXPIRES_IN` are properly configured

#### For Developers
```javascript
// New token payload structure (generated by login/register)
{
  id: 123,           // User ID
  user_id: 123,      // Backward compatibility
  email: "user@example.com",
  iat: 1234567890,   // Issued at
  exp: 1234999999    // Expires at
}

// Middleware sets req.user with normalized structure
req.user = {
  id: 123,
  user_id: 123,
  email: "user@example.com",
  raw: { /* full decoded token */ }
}
```

#### Testing
New comprehensive test suite added in `tests/auth.token.test.js` covers:
- Token generation with user ID
- Multiple token payload formats
- Edge cases (expired tokens, missing ID, invalid tokens)
- Optional authentication middleware behavior
